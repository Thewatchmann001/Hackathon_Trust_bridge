...
Fetches internship opportunities via RapidAPI.
"""
import http.client
import json
...
class InternshipsProvider(BaseJobProvider):
    """Internships provider via RapidAPI."""
    
    def __init__(self):
        super().__init__("Internships")
        self.host = "internships-api.p.rapidapi.com"
        self.timeout = 10
        self.rapidapi_key = getattr(settings, 'RAPIDAPI_KEY', None)
        
        if not self.rapidapi_key:
            self.enabled = False
            logger.warning("RapidAPI key not configured - Internships provider disabled")
    ...
    def _make_request(self, conn, endpoint: str, headers: dict) -> dict:
        ...
        try:
            ...
            if response.status != 200:
                logger.warning(f"Internships API returned status {response.status}")
                return {}
            
            data = json.loads(response.read().decode())
            return data
        except Exception as e:
            logger.error(f"Internships API request failed: {e}")
            return {}
    
    def normalize_job(self, raw_job: Dict[str, Any]) -> Optional[JobSchema]:
        """Normalize internship to JobSchema."""
        try:
            ...
            job_id = f"internship_{raw_job.get('id', '')}"
            
            return JobSchema(
                id=job_id,
                title=raw_job.get('title', 'Untitled Internship'),
                ...
                metadata={
                    "job_type": "Internship",
                    "paid": raw_job.get('paid', False)
                }
            )
        except Exception as e:
            self.logger.warning(f"Failed to normalize internship: {e}")
            return None
